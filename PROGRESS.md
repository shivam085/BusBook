...
## 💳 Phase 7: Payment Gateway, PDF Tickets, & Emails
**Goal:** Securely process payments via Razorpay and automatically dispatch E-Tickets to the user's inbox.
- **Payment Architecture (Phase 7A):**
  - Refactored bookings to initially save as `pending`.
  - Integrated the **Razorpay SDK** in a Server-to-Server handshake (Backend creates order -> Frontend processes via UI popup -> Backend cryptographically verifies the HMAC SHA256 signature).
  - Built a robust **Mock Mode Bypass**: If API keys aren't provided, the app simulates a successful transaction alert in the browser so development/testing is never blocked.
- **E-Tickets & PDF Generation (Phase 7B):**
  - Implemented `pdfkit` in `ticket.service.js` to draw a beautiful, customized E-Ticket dynamically in the server's RAM (Memory Buffer).
  - Created a secure, protected API endpoint (`GET /api/bookings/:id/ticket`) that pipes this binary PDF buffer directly to the browser for downloading.
  - Updated the "My Bookings" UI with a "Download E-Ticket" button that securely triggers the binary blob download via axios.
- **Email Dispatching:**
  - Integrated `nodemailer` alongside `mailgen`.
  - Built an `email.service.js` that compiles an elegant, universally compatible HTML email template using Mailgen's default theme.
  - Configured asynchronous processing: Upon successful payment verification, the server creates the PDF in memory, attaches it to the Mailgen template, and fires it directly to the user's inbox without blocking the API response.
